...
struct ConnectionConfigView: View {
    ...
    var body: some View {
        ZStack {
            List {
                // EMBEDDED NATIVE WIREGUARD VPN SECTION
                Section(header: Text("EMBEDDED NATIVE WIREGUARD VPN")) {
                    HStack {
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Tunnel Status")
                                .font(.system(size: 16, weight: .medium))
                            Text(VPNManager.shared.status.description)
                                .font(.system(size: 13))
                                .foregroundColor(.secondary)
                        }
                        Spacer()
                        Circle()
                            .fill(VPNManager.shared.isConnected ? Color.green : (VPNManager.shared.isConnecting ? Color.orange : Color.red))
                            .frame(width: 12, height: 12)
                    }
                    
                    Button(action: {
                        VPNManager.shared.toggleTunnel()
                    }) {
                        HStack {
                            Spacer()
                            Text(VPNManager.shared.isConnected ? "Disconnect Embedded Tunnel" : (VPNManager.shared.isConnecting ? "Connecting..." : "Connect Embedded Tunnel"))
                                .fontWeight(.semibold)
                                .foregroundColor(VPNManager.shared.isConnected ? .red : .accentColor)
                            Spacer()
                        }
                    }
                    
                    Toggle("Auto-Connect on App Launch", isOn: Binding(
                        get: { VPNManager.shared.isAutoVPNEnabled },
                        set: { VPNManager.shared.isAutoVPNEnabled = $0 }
                    ))
                    
                    Toggle("Disconnect in Background", isOn: Binding(
                        get: { VPNManager.shared.isAutoDisconnectOnBackgroundEnabled },
                        set: { VPNManager.shared.isAutoDisconnectOnBackgroundEnabled = $0 }
                    ))
                }
