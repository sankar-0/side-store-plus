...
final class AppManager: ObservableObject, @unchecked Sendable
{
    ...
    func deactivateApps(for appBundle: ALTApplication, presentingViewController: UIViewController?, completion: @escaping (Result<Void, Error>) -> Void)
    {
        ...
        DispatchQueue.main.async {
            ...
            let usableLimit = MultiAccountManager.shared.totalUsableAppSlots
            
            var title: String = activeAccountsCount > 1
                ? String(format: NSLocalizedString("Cannot Activate More than %d Apps (%d Accounts)", comment: ""), usableLimit, activeAccountsCount)
                : NSLocalizedString("Cannot Activate More than 3 Apps", comment: "")
            let message: String
            
            if activeAccountsCount > 1
            {
                message = String(format: NSLocalizedString("Your configured %d Apple ID accounts have reached their combined limit of %d active apps. Please choose an app to deactivate.", comment: ""), activeAccountsCount, usableLimit)
            }
