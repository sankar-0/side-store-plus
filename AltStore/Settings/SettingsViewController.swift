...
private extension SettingsViewController
{
    ...
    func update()
    {
        ...
        {
            let accountsCount = MultiAccountManager.shared.activeAccountsCount
            if accountsCount > 1 {
                self.accountNameLabel.text = team.name
                self.accountEmailLabel.text = team.account.appleID
                self.accountTypeLabel.text = String(format: NSLocalizedString("%@ • %d Accounts Active (%d Usable Slots)", comment: ""), team.type.localizedDescription, accountsCount, MultiAccountManager.shared.totalUsableAppSlots)
            } else {
                self.accountNameLabel.text = team.name
                self.accountEmailLabel.text = team.account.appleID
...
private extension SettingsViewController
{
    func openMultiAccountView()
    {
        let multiAccountView = MultiAccountView()
        let hostingVC = UIHostingController(rootView: multiAccountView)
        let appearance = UINavigationBarAppearance()
        appearance.configureWithDefaultBackground()
        hostingVC.navigationItem.scrollEdgeAppearance = appearance
        hostingVC.navigationItem.standardAppearance = appearance
...
extension SettingsViewController
{
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        case .account: self.openMultiAccountView()
        case .appRefresh:
