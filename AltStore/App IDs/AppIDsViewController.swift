...
private extension AppIDsViewController
{
    ...
    func footerText() -> String {
        ...
        let activeAccounts = MultiAccountManager.shared.activeAccountsCount
        if activeAccounts > 1 {
            let totalQuota = MultiAccountManager.shared.totalWeeklyAppIDQuota
            return String(format: NSLocalizedString("%@ of %@ App IDs Used (%d Accounts)", comment: ""), NSNumber(value: count), NSNumber(value: totalQuota), activeAccounts)
        }
        return count == 1
...
extension AppIDsViewController: UICollectionViewDelegateFlowLayout
{
    ...
    override func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView
    {
        ...
        case UICollectionView.elementKindSectionHeader:
            ...
            {
                let activeAccounts = MultiAccountManager.shared.activeAccountsCount
                let quotaSubtitle = activeAccounts > 1
                    ? String(format: NSLocalizedString("Apple limits non-developer Apple IDs to 10 App IDs at a time. With %d active accounts configured, your pool has a total quota of %d App IDs per week.", comment: ""), activeAccounts, MultiAccountManager.shared.totalWeeklyAppIDQuota)
                    : NSLocalizedString("Apple limits non-developer Apple IDs to 10 App IDs at a time.", comment: "")
