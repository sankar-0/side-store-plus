...
private extension MyAppsViewController
{
    ...
    {
        var message: String
        let activeAccountsCount = MultiAccountManager.shared.activeAccountsCount
        let maxUsableSlots = MultiAccountManager.shared.totalUsableAppSlots
        let totalSystemSlots = MultiAccountManager.shared.totalSystemSlots
        
        if activeAccountsCount > 1
        {
            message = String(format: NSLocalizedString("With %d active Apple IDs configured in your Multi-Account pool, you have %d usable app slots (%d system slots total, with 1 slot reserved for SideStore on the Primary Account).\n\nInactive apps are backed up and uninstalled so they don't count towards your total, but will be reinstalled with all their data when activated again.", comment: ""), activeAccountsCount, maxUsableSlots, totalSystemSlots)
        }
        else if UserDefaults.standard.activeAppLimitIncludesExtensions
        {
